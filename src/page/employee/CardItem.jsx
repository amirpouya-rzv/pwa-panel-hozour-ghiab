...
function CardItem({ card, onClick, onDelete }) {
  ...
  return (
    <div
      className="min-w-[240px] md:min-w-0 dark:bg-darkgray bg-white border-2 mb-5 border-lightgray shadow hover:scale-105 transition rounded-3xl  cursor-pointer"
      onClick={onClick}
    >
      <span>
        <button
          className="md:mx-[320px] mx-96 md:pt-5 mt-2 text-lightred text-3xl"
          onClick={(e) => {
            e.stopPropagation();
            onDelete();
          }}
        ...
        <div className="flex flex-col justify-center   items-center">
          {card.photo_url ? (
            <img
              src={card?.photo_url}
              alt="employee"
              className="w-[100px] h-[100px] flex justify-center mx-36 mb-5 md:mx-52 rounded-full object-cover border-2 border-blue dark:border-black"
            />
          ) : (
...
function InfoMini({ label, value }) {
  return (
    <div>
      ...
      <p className="text-[12px] flex justify-center border-2 dark:border dark:border-lightgray dark:bg-stone-600 dark:text-white border-blue rounded-full md:px-14 py-2 mt-1 overflow-x-auto whitespace-nowrap">
        {value}
      </p>
    </div>
  );
